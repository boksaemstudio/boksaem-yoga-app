...
async function fix() {
    ...
    const now = new Date();
    const classTpl = [
        { time: '07:00', title: 'ëª¨ë‹ ë¹ˆì•¼??, instructor: '? ë§ˆ ?ì¥', level: '1', duration: 60 },
        { time: '10:00', title: 'ê¸°êµ¬ ?„ë¼?ŒìŠ¤', instructor: 'ë£¨ì‹œ ê°•ì‚¬', level: '1.5', duration: 60 },
        { time: '14:00', title: '?ë§?”ê?', instructor: '?Œí”¼ ì§€?ì¥', level: '0.5', duration: 60 },
        { time: '19:00', title: 'ì½”ì–´ ?¸í…?œë¸Œ', instructor: '?¬ë¦¬ë¹„ì•„ ê°•ì‚¬', level: '2', duration: 60 },
        { time: '21:00', title: '?€???˜í?', instructor: '?¬ë¦¬ë¹„ì•„ ê°•ì‚¬', level: '1', duration: 60 }
    ];
    ...
    await db.doc(`studios/${TARGET}`).set({
        ...
        settings: {
            ...
            MEMBERSHIP: {
                TYPES: { 'MTypeA': 'ê¸°êµ¬?„ë¼?ŒìŠ¤ 30?Œê¶Œ', 'MTypeB': '?Œë¼?‰ìš”ê°€ 1ê°œì›”ê¶?, 'MTypeC': 'ë¹ˆì•¼??ë¬´ì œ???¨ìŠ¤' }
            },
