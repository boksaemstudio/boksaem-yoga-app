// 일회성 가격표 데이터 Firestore 저장 스크립트
// 실행: node scripts/seed_pricing.js
...
const pricingData = {
    ...
    // 메타 데이터 (결제 정보, 할인 안내, 연기 규칙)
    _meta: {
        payment: {
            bank: '하나은행',
            account: '379-910319-22507',
            holder: '복샘요가(김복순)'
        },
...
async function seedPricing() {
    try {
        ...
        console.log(`   카테고리: ${Object.keys(pricingData).filter(k => k !== '_meta').join(', ')}`);
        process.exit(0);
    } catch (e) {
