...
2. 빌드: `npm run build`
3. CLI 배포 (정식앱 + 메인 도메인): `firebase deploy --only hosting:boksaem-yoga,hosting:passflowai,hosting:passflow-landing`
4. 기타 데모/테스트 도메인 배포 (REST API): `node functions/deploy_rest_api.cjs`
5. Git 커밋/푸시: `git add -A; git commit -m "deploy"; git push`

## 참고
- boksaem-yoga, passflowai, passflow-landing은 CLI로 배포 가능 (firebase.json rewrites 정상 작동)
