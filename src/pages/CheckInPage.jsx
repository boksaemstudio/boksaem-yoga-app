...
const CheckInPage = () => {
  ...
  const [loadingMessage, setLoadingMessage] = useState(t('checkin_verifying') || t("g_a03db7") || t("g_a03db7") || t("g_a03db7") || t("g_a03db7") || t("g_a03db7") || "\uCD9C\uC11D \uD655\uC778 \uC911...");
  ...
  const loadAIExperience = async (name = "\uBC29\uBB38 \uD68C\uC6D0", credits = null, days = null, w = null) => {
    ...
    try {
      ...
      const title = info?.title || t("g_2b3da3") || t("g_2b3da3") || t("g_2b3da3") || t("g_2b3da3") || t("g_2b3da3") || "\uC790\uC728\uC218\uB828";
      if (isStandby) {
        ...
        setAiLoading(true);
        storageService.getAIExperience(name, 0, t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || "\uC624\uB298", h, title, w, null, null, language, null, 'visitor', 'checkin').then(res => {
          if (res?.message && !res.isFallback) setAiEnhancedMsg(res.message.replace(/나마스테[.]?\s*🙏?/gi, '').trim());
        }).finally(() => setAiLoading(false));
      } else {
        const exp = await storageService.getAIExperience(name, 0, t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || t("g_e1e8a7") || "\uC624\uB298", h, title, w, credits, days, language, null, 'member', 'checkin');
  ...
  const fetchWeatherAndAI = async () => {
    try {
      ...
      loadAIExperience(t("g_403daa") || t("g_403daa") || t("g_403daa") || t("g_403daa") || t("g_403daa") || "\uBC29\uBB38 \uD68C\uC6D0", null, null, d.current_weather);
    } catch (e) {
      loadAIExperience();
    }
  };
  ...
  const showCheckInSuccess = async (res, isDup = false) => {
    ...
    setAiEnhancedMsg(t('checkin_ai_peaceful') || t("g_224835") || t("g_224835") || t("g_224835") || t("g_224835") || t("g_224835") || "\uC624\uB298\uB3C4 \uD3C9\uC628\uD55C \uC694\uAC00 \uC548\uB0B4\uD574 \uB4DC\uB9B4\uAC8C\uC694. \uB098\uB9C8\uC2A4\uD14C \uD83D\uDE4F");
    setAiLoading(false);
  };
      ...
      try {
        ...
        if (members) {
          setFaceConfirmMember({
            id: memberIdToForce,
            name: members.name || t("g_6745df") || t("g_6745df") || t("g_6745df") || t("g_6745df") || t("g_6745df") || "\uD68C\uC6D0"
          });
          return;
        }
    ...
    try {
      ...
      if (!targetMemberId) {
        ...
        if (members.length === 0) {
          ...
          setMessage({
            type: 'error',
            text: t('checkin_member_not_found')
          });
          return;
        }
        if (members.length > 1) {
