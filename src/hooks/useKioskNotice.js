 ...
 */
export function useKioskNotice({
  isReady,
  currentBranch,
  message,
  showSelectionModal,
  ...
  useEffect(() => {
    ...
    unsubscribeBranch = storageService.subscribeToKioskSettings(currentBranch, branchData => {
      if (branchData) setRawKioskSettings(branchData);else setRawKioskSettings({});
  ...
  useEffect(() => {
    ...
    resetNoticeTimer();
    window.addEventListener('touchstart', resetNoticeTimer, {
      passive: true
    });
    window.addEventListener('mousedown', resetNoticeTimer, {
      passive: true
    });
    window.addEventListener('keydown', resetNoticeTimer, {
      passive: true
    });
    return () => {
      clearTimeout(idleTimer);
