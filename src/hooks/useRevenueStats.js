  ...
  return useMemo(() => {
    ...
    (sales || []).forEach(s => {
      ...
      if (currentBranch !== 'all') {
        ...
        const matchFound = saleBranch && saleBranch === currentBranch || !saleBranch && memberBranch && memberBranch === currentBranch;
      ...
      if (typeof rawDate === 'string' && rawDate.includes('T')) {
        ...
        dateStr = rawDate.toDate().toLocaleDateString('sv-SE', {
          timeZone: 'Asia/Seoul'
        });
      } else {
    ...
    finalItems.forEach(item => {
      ...
      if (item.isNew) current.newCount += 1;else current.reregCount += 1;
      membershipMap.set(key, current);
    });
    const membershipSales = Array.from(membershipMap.values()).sort((a, b) => b.count - a.count);
    return {
