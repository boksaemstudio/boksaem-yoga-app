...
export const useMemberProfile = (language, t) => {
  ...
  const getTraditionalYogaMessage = useCallback(() => {
    ...
    let timeMsg = "";
    if (hour >= 5 && hour < 9) timeMsg = t('trad_dawn');else if (hour >= 9 && hour < 12) timeMsg = t('trad_morning');else if (hour >= 12 && hour < 17) timeMsg = t('trad_afternoon');else if (hour >= 17 && hour < 21) timeMsg = t('trad_evening');else timeMsg = t('trad_night');
    let seasonMsg = "";
    if (month >= 3 && month <= 5) seasonMsg = t('season_spring');else if (month >= 6 && month <= 8) seasonMsg = t('season_summer');else if (month >= 9 && month <= 11) seasonMsg = t('season_autumn');else seasonMsg = t('season_winter');
    ...
    try {
      ...
      if (attendDates.length >= 3) {
        ...
        attendDates.forEach(d => {
          const w = Math.floor((nowMs - new Date(d).getTime()) / WEEK);
          if (w >= 0 && w < 4) recentWeeks[w] = 1;
        });
      ...
      }, 'profile', m.mbti || localStorage.getItem('member_mbti') || null);
      if (exp) {
        setAiExperience(exp);
        storageService.setGreetingCache(m.id, {
          ...exp,
          _cachedForName: m.name
        });
      }
    } catch (e) {
  ...
  const loadMemberData = useCallback(async memberId => {
    try {
      setLoading(true);
      const [memberData, history, noticeData, imagesData, messagesData] = await Promise.all([storageService.fetchMemberById(memberId), storageService.getAttendanceByMemberId(memberId), storageService.loadNotices(), storageService.getImages(), storageService.getMessagesByMemberId(memberId)]);
      if (memberData) {
        setMember(prev => ({
          ...memberData,
        ...
        if (lastAiAnalysisArgs.current !== currentAnalysisArgs) {
          ...
          storageService.getAIAnalysis(memberData.name, validHistory.length, validHistory, getKSTHour(), language, 'member').then(analysis => setAiAnalysis(analysis)).catch(() => setAiAnalysis({
  ...
  useEffect(() => {
    ...
    const q = query(tenantDb.collection('attendance'), where('memberId', '==', memberId), orderBy('timestamp', 'desc'), firestoreLimit(logLimit));
  ...
  useEffect(() => {
    ...
    const allMessages = [...individualMessages, ...noticeMessages].sort((a, b) => new Date(b.timestamp || 0) - new Date(a.timestamp || 0));
  ...
  useEffect(() => {
    ...
    let msgUnsub = () => {};
    if (member?.id) {
      const q = query(tenantDb.collection('messages'), where('memberId', '==', member.id), orderBy('timestamp', 'desc'), firestoreLimit(30));
      msgUnsub = onSnapshot(q, snap => {
        setIndividualMessages(snap.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
  ...
  useEffect(() => {
    if (member) {
      ...
      lastAiAnalysisArgs.current = currentAnalysisArgs;
      storageService.getAIAnalysis(member.name, validLogs.length, validLogs, getKSTHour(), language, 'member').then(analysis => setAiAnalysis(analysis)).catch(() => setAiAnalysis({
        message: t('analysisPending'),
        isError: true
      }));
      if (notices.length > 0) {
