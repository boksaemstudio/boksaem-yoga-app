...
export function useMemberUI() {
  ...
  const openConfirmModal = useCallback((message, onConfirm, isConfirm = false) => {
    ...
    if (!window.history.state?.modalOpen) window.history.pushState({
      modalOpen: true
    }, '');
  }, []);
  ...
  const safeSetLightboxImage = useCallback(img => {
    if (img && !window.history.state?.modalOpen) window.history.pushState({
      modalOpen: true
  ...
  const safeSetSelectedNoticeId = useCallback(id => {
    if (id && !window.history.state?.modalOpen) window.history.pushState({
      modalOpen: true
