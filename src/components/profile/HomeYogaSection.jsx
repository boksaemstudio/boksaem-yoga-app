        ...
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <div style={{
                    ...
                    borderRadius: '12px', border: '1px solid rgba(var(--primary-rgb), 0.2)',
                }}>
                    {pose.emoji}
                </div>
                <div>
                    <div style={{ fontWeight: 'bold', fontSize: '0.95rem', color: 'white' }}>{pose.name}</div>
                    <div style={{ fontSize: '0.78rem', color: 'var(--primary-gold)' }}>{pose.benefit}</div>
                </div>
            </div>
            <div style={{
                fontSize: '0.85rem',
...
const HomeYogaSection = ({ language, t, mbti, weatherData, logs }) => {
    ...
    const weatherInfo = useMemo(() => {
        if (!weatherData) return { weather: null, temp: null };
        return {
        ...
        return (
            <div style={{ marginBottom: '25px' }}>
                <div style={{
                    ...
                    <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', padding: '10px', textAlign: 'center' }}>
                        🧘 {localT('aiRecommendLoading') || 'AI 추천 로딩 중...'}
                    </div>
                </div>
            </div>
        );
    ...
    const contextBadges = [];
    if (mbti) contextBadges.push({ label: mbti, icon: '🧬' });
    if (weatherInfo.weather) {
        const weatherEmoji = weatherInfo.weather.includes('흐림') || weatherInfo.weather.includes('cloud') ? '☁️'
            : weatherInfo.weather.includes('비') || weatherInfo.weather.includes('rain') ? '🌧️'
            : weatherInfo.weather.includes('눈') || weatherInfo.weather.includes('snow') ? '❄️'
            : '☀️';
        const tempStr = weatherInfo.temp ? `${Math.round(weatherInfo.temp)}°` : '';
        contextBadges.push({ label: `${weatherEmoji} ${tempStr}`, icon: null });
    }
    if (primaryClass) contextBadges.push({ label: primaryClass, icon: '🏷️' });
