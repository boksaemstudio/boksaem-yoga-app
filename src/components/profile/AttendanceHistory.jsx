...
const AttendanceHistory = ({
  ...
  useEffect(() => {
    ...
    const messages = [t('loadMsg1'), t('loadMsg2'), t('loadMsg3'), t('loadMsg4')];
    let i = 0;
      ...
      days.push(<div key={day} style={{
        ...
        alignItems: 'center'
          ...
          marginBottom: '4px'
                    ...
                    {/* Holiday Label */}
                    {getHolidayName(dateStr) && <div style={{
          fontSize: '0.6rem',
          color: '#ff6b6b',
          marginBottom: '2px',
          overflow: 'hidden',
  ...
  return <div className="fade-in">
      ...
      border: '1px solid rgba(255,255,255,0.1)'
          ...
          gap: '10px'
            ...
            color: 'rgba(255,255,255,0.5)'
              ...
              n: validLogs.length
                            ...
                            {expiredLogsCount > 0 && <span style={{
              color: '#ff4d4f',
              marginLeft: '4px'
            }}>({t('expiredCancelCount', {
                n: expiredLogsCount
              })})</span>}
                        </span>
                    </div>
        ...
        gap: '12px'
          ...
          if (isExpired) {
            ...
            displayBadge = t('sessionN', {
              n: validLogs.length - validIndex
            });
          }
          return <div key={log.id} style={{
            ...
            alignItems: 'center'
                                    ...
                                    <div>
                                        <div style={{
                fontSize: '0.8rem',
                color: isExpired ? '#ff4d4f' : 'var(--primary-gold)'
              }}>
                                            {isValidDate ? <>
                                                    {new Intl.DateTimeFormat(language === 'ko' ? 'ko-KR' : language === 'en' ? 'en-US' : language === 'ru' ? 'ru-RU' : language === 'zh' ? 'zh-CN' : 'ja-JP', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
                  }).format(date)} {date.toLocaleTimeString(language === 'ko' ? 'ko-KR' : language === 'en' ? 'en-US' : language === 'ru' ? 'ru-RU' : language === 'zh' ? 'zh-CN' : 'ja-JP', {
                    hour12: false,
                    hour: '2-digit',
                    minute: '2-digit'
                  })}
                ...
                fontSize: '1rem',
                fontWeight: 'bold',
                color: isExpired ? '#rgba(255,255,255,0.5)' : 'white',
              ...
              gap: '8px'
                                        ...
                                        {localStorage.getItem('watchSync') === 'true' && !isExpired && <button onClick={() => setSelectedReportLog(log)} style={{
                ...
                borderRadius: '10px'
                                        ...
                                        {onDelete && <button onClick={() => onDelete(log.id)} disabled={isSubmitting} style={{
                background: isSubmitting ? 'rgba(100,100,100,0.2)' : 'rgba(239, 68, 68, 0.2)',
                color: isSubmitting ? '#888' : '#ef4444',
                border: 'none',
                borderRadius: '6px',
                padding: '8px',
                ...
                transition: 'opacity 0.2s'
                        ...
                        {setLogLimit && logs.length >= logLimit && <button onClick={() => setLogLimit(prev => prev + 10)} style={{
          width: '100%',
          padding: '12px',
          ...
          opacity: 0.3
                    ...
                    </div> : (/* Calendar View Implementation */
      <div className="fade-in">
          ...
          marginBottom: '15px'
                            ...
                            {(config.BRANCHES || []).map(branch => <div key={branch.id} style={{
            display: 'flex',
            alignItems: 'center',
            gap: '6px'
          }}>
          ...
          gap: '5px'
            ...
            {/* Health Report Modal */}
            {selectedReportLog && <WorkoutReportModal log={selectedReportLog} onClose={() => setSelectedReportLog(null)} t={t} />}
        </div>;
};
