...
const Keypad = memo(({
  ...
  return <div className="keypad-grid">
            ...
            <KeyButton value="0" onPress={onKeyPress} disabled={disabled} style={{
      gridColumn: 'span 2'
    }}>
                0
            </KeyButton>

            <KeyButton onPress={onClear} disabled={disabled} className="clear-btn">
                <div style={{
        fontSize: '2.5rem',
        fontWeight: 'bold'
      }}>{t("g_5435ef") || "Clear"}</div>
            </KeyButton>
        </div>;
});
...
const KeyButton = memo(({
  ...
  const handleAction = useCallback(e => {
    ...
    if (value !== undefined) onPress(value);else onPress();
  ...
  return <button className={`keypad-btn ${className || ''} ${special || ''}`}
  ...
  onTouchStart={e => {
    ...
    addActiveStyle(e);
    handleAction(e);
  }} onTouchEnd={removeActiveStyle} onTouchCancel={removeActiveStyle}
  // [FIX] PC 마우스 클릭 시
  onClick={e => {
    if (disabled) return;
    ...
    handleAction(e);
  }} onMouseDown={addActiveStyle} onMouseUp={removeActiveStyle} onMouseLeave={removeActiveStyle} disabled={disabled} style={{
