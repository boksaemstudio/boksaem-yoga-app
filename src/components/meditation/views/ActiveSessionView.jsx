...
export const ActiveSessionView = ({
    ...
    interactionType, isPlaying, formatTime, timeLeft, micVolume, ttcEnabled, permissionError,
    completeSession, togglePlay, showVolumePanel, setShowVolumePanel, soundEnabled, setSoundEnabled,
    setAudioVolumes, currentAudioRef, updateAmbientVolume, updateBinauralVolume,
    videoRef, canvasRef, activeMode, showVolumeHint
}) => {
    return (
        <div style={{ position: 'fixed', inset: 0, background: config.THEME?.BACKGROUND || '#0a0a0c', zIndex: 3000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {interactionType === 'v3' && (
                <div style={{ position: 'absolute', inset: 0, zIndex: 1, opacity: 0.6 }}>
                    <video ref={videoRef} autoPlay playsInline muted style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scaleX(-1)' }} />
                    <canvas ref={canvasRef} width={window.innerWidth} height={window.innerHeight} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', transform: 'scaleX(-1)', pointerEvents: 'none' }} />
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '280px', height: '350px', border: '1px solid rgba(255, 215, 0, 0.3)', borderRadius: '120px', boxShadow: '0 0 80px rgba(255, 215, 0, 0.1) inset' }} />
                </div>
            )}

            <div className={`soul-light-base soul-theme-${visualTheme} ${isPlaying && interactionType !== 'v2' ? 'active' : ''}`} 
                style={{ transform: interactionType === 'v2' && isPlaying ? `translate(-50%, -50%) scale(${1 + Math.min(micVolume, 0.5)})` : undefined, transition: interactionType === 'v2' ? 'transform 0.1s ease-out' : 'all 1s ease', zIndex: 0 }} />
            ...
            <div style={{ zIndex: 10, textAlign: 'center', width: '100%', padding: '20px', maxWidth: '600px' }}>
                <div style={{ marginBottom: '20px', minHeight: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <p style={{ color: 'white', fontSize: '1.2rem', fontWeight: 500, lineHeight: 1.6, opacity: isPlaying ? 1 : 0.5, transition: 'opacity 1s ease', textShadow: '0 4px 20px rgba(0,0,0,0.8)', background: interactionType === 'v3' ? 'rgba(0,0,0,0.6)' : 'transparent', padding: interactionType === 'v3' ? '15px' : '0', borderRadius: '15px', wordBreak: 'keep-all' }}>
                ...
                {interactionType === 'v2' && (
                    <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', animation: 'fadeIn 0.5s ease' }}>
                        ...
                        <div style={{ width: '120px', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', overflow: 'hidden', boxShadow: '0 0 10px rgba(0,0,0,0.5)' }}>
                            <div style={{ height: '100%', width: `${Math.min(micVolume * 100, 100)}%`, background: `linear-gradient(90deg, ${config.THEME?.SUCCESS_COLOR || '#4ade80'}, #32ff7e)`, transition: 'width 0.1s ease-out', boxShadow: '0 0 10px rgba(74, 222, 128, 0.5)' }} />
                        </div>
                        {micVolume > 0.1 && (
            ...
            <div style={{ position: 'absolute', bottom: '60px', display: 'flex', alignItems: 'center', gap: '40px', zIndex: 20 }}>
                <button onClick={() => completeSession()} style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                    <X size={28} />
                </button>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    <button onClick={() => setShowVolumePanel(!showVolumePanel)} style={{ width: '60px', height: '60px', borderRadius: '50%', background: showVolumePanel ? 'rgba(var(--primary-rgb), 0.3)' : 'rgba(255,255,255,0.1)', border: 'none', color: soundEnabled ? 'white' : 'rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}>
            ...
            {showVolumeHint && !showVolumePanel && (
                <div style={{ position: 'absolute', bottom: '190px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(var(--primary-rgb), 0.2)', backdropFilter: 'blur(10px)', border: '1px solid rgba(var(--primary-rgb), 0.3)', borderRadius: '20px', padding: '8px 16px', color: 'rgba(255,255,255,0.8)', fontSize: '0.75rem', whiteSpace: 'nowrap', animation: 'fadeIn 0.5s ease-out', pointerEvents: 'none' }}>
