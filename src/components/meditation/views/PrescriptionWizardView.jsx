...
const ICON_MAP = {
  ...
  Microphone,
  VideoCamera
}; // Expanded inline for self-containment
      ...
      overflow: 'hidden'
        ...
        justifyContent: 'center'
          ...
          textAlign: 'center'
            ...
            color: 'white',
            marginTop: '20px'
          }}>{t('med_wiz_title_ai_analysis') || t("g_c8423c") || "AI \uB9C8\uC74C \uBD84\uC11D"}</h2>
                    </div>
          ...
          boxShadow: '0 10px 20px rgba(var(--primary-rgb), 0.3)'
                        ...
                        {t('med_wiz_btn_select_mode') || t("g_b70b05") || "\uBA85\uC0C1 \uBAA8\uB4DC \uC120\uD0DD\uD558\uAE30"}
                    </button>
                </div>
            </div>;
  }
      ...
      overflow: 'hidden'
        ...
        width: '100%'
          ...
          textAlign: 'center'
            ...
            fontWeight: 800,
            color: 'white'
          }}>{t('med_wiz_title_select_intent') || t("g_cb63e4") || "\uC5B4\uB5A4 \uBA85\uC0C1\uC744 \uC6D0\uD558\uC2DC\uB098\uC694?"}</h2>
                        <p style={{
            color: 'rgba(255,255,255,0.5)',
            marginTop: '8px'
          }}>{t('med_wiz_desc_select_intent') || t("g_29ec81") || "\uB098\uC5D0\uAC8C \uAC00\uC7A5 \uD544\uC694\uD55C \uBAA8\uB4DC\uB97C \uC120\uD0DD\uD558\uC138\uC694"}</p>
                    </div>
          ...
          margin: '0 auto'
            ...
            gap: '20px',
            padding: '25px',
            background: interactionType === t.id ? 'rgba(var(--primary-rgb), 0.15)' : 'rgba(255,255,255,0.05)',
            border: interactionType === t.id ? '1px solid var(--primary-gold)' : '1px solid rgba(255,255,255,0.1)',
            borderRadius: '24px',
            color: 'white',
            textAlign: 'left',
            cursor: 'pointer',
          ...
          width: '100%'
            ...
            marginBottom: '20px'
              ...
              fontSize: '0.85rem',
              marginBottom: '10px'
            }}>⏱️ {t('med_wiz_duration_title') || t("g_8c6297") || "\uBA85\uC0C1 \uC2DC\uAC04"}</div>
                            <div style={{
              display: 'flex',
              gap: '8px'
            }}>
                                {MEDITATION_MODES.map(m => <button key={m.id} onClick={() => {
                setActiveMode(m);
                setTimeLeft(m.time);
              }} style={{
                flex: 1,
                ...
                border: activeMode && activeMode.id === m.id ? '1px solid rgba(255,255,255,0.3)' : '1px solid rgba(255,255,255,0.05)',
                fontWeight: 600
              }}>{t(`med_mode_${m.id}_short`) || m.label.split(' ')[0]}</button>)}
                            </div>
                        </div>
              ...
              fontSize: '0.85rem',
              marginBottom: '10px'
            }}>🎵 {t('med_wiz_ambient_title') || t("g_53388d") || "\uC790\uC5F0\uC74C \uBC30\uACBD (\uCF1C\uAE30/\uB044\uAE30)"}</div>
                            <div style={{
              display: 'flex',
              gap: '8px',
              flexWrap: 'wrap'
            }}>
                                {AMBIENT_SOUNDS.map(a => <button key={a.id} onClick={() => setSelectedAmbient(a.id)} style={{
                padding: '8px 16px',
                borderRadius: '16px',
                fontSize: '0.85rem',
                ...
                fontWeight: 600,
                cursor: 'pointer'
              }}>{t(`med_ambient_${a.id}`) || a.label}</button>)}
                            </div>
                        </div>
            ...
            marginTop: '40px'
              ...
              setStep('preparation');
              setPrepStep(3);
            }} disabled={!interactionType} style={{
              width: '100%',
              background: interactionType ? 'var(--primary-gold)' : 'rgba(255,255,255,0.1)',
              color: interactionType ? 'var(--text-on-primary)' : 'rgba(255,255,255,0.3)',
              padding: '18px',
              borderRadius: '18px',
              fontSize: '1.2rem',
              fontWeight: 800,
              border: 'none',
              cursor: interactionType ? 'pointer' : 'not-allowed',
              boxShadow: interactionType ? '0 10px 20px rgba(var(--primary-rgb), 0.3)' : 'none',
              transition: 'all 0.3s'
            }}>
                                {interactionType ? t('med_wiz_btn_start_prep') || t("g_387e7e") || "\uBA85\uC0C1 \uC900\uBE44\uD558\uAE30" : t('med_wiz_btn_need_selection') || t("g_c7dcff") || "\uC6D0\uD558\uC2DC\uB294 \uBA85\uC0C1\uC744 \uC120\uD0DD\uD574\uC8FC\uC138\uC694"}
                            </button>
                        </div>
                    </div>
                </div>
      ...
      overflow: 'hidden'
        ...
        overflowY: 'auto'
          ...
          paddingBottom: '40px'
            ...
            marginBottom: '20px',
            color: 'var(--primary-gold)'
          }}><Sparkle size={48} weight="fill" /></div>
                        <h2 style={{
            fontSize: '1.4rem',
            fontWeight: 700,
            color: 'white',
            ...
            textAlign: 'center'
                            ...
                            {t('med_wiz_title_ai_prescription') || t("g_832e46") || "\uBA85\uC0C1 \uC804\uBB38 AI \uCC98\uBC29"}
                        </h2>
            ...
            gap: '15px'
              ...
              lineHeight: 1.6
                ...
                marginBottom: '8px',
                fontSize: '0.85rem'
              }}>📋 {t('med_wiz_ai_analysis_res') || t("g_ad5238") || "AI \uC2EC\uB9AC \uBD84\uC11D \uACB0\uACFC"}</div>
              ...
              gap: '15px'
                ...
                height: '60px',
                borderRadius: '18px',
                background: `${activeMode.color}30`,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                color: activeMode.color
                  ...
                  fontWeight: 600,
                  marginBottom: '2px'
                }}>✨ {t('med_wiz_ai_recommendation') || t("g_500870") || "AI \uAC15\uB825 \uCD94\uCC9C"}</div>
                                    <div style={{
                  fontSize: '1.25rem',
                  fontWeight: 700,
                  color: 'white'
                  ...
                  color: 'rgba(255,255,255,0.6)'
                                        ...
                                        {interactionType === 'v1' && (t('med_wiz_type_bodyscan') || t("g_26a7df") || "\uC628\uBAB8 \uC774\uC644 \uAC00\uC774\uB4DC")}
                                        {interactionType === 'v2' && (t('med_wiz_type_breath') || t("g_deb8a5") || "\uD638\uD761 \uBAB0\uC785")}
                                        {interactionType === 'v3' && (t('med_wiz_type_posture') || t("g_8c78ca") || "\uC790\uC138 \uAD50\uC815")}
                                    </div>
                                </div>
                            </div>
              ...
              marginTop: '10px'
                ...
                fontSize: '0.75rem',
                marginBottom: '10px'
              }}>{t('med_wiz_change_options') || t("g_a8bf03") || "\uC635\uC158 \uBCC0\uACBD\uD558\uAE30"}</div>
                ...
                gap: '8px'
                                    ...
                                    {Object.values(INTERACTION_TYPES).map(tOpt => <button key={tOpt.id} onClick={() => setInteractionType(tOpt.id)} style={{
                  flex: 1,
                  padding: '8px',
                  borderRadius: '10px',
                  fontSize: '0.75rem',
                  background: interactionType === tOpt.id ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.05)',
                  color: interactionType === tOpt.id ? 'white' : 'rgba(255,255,255,0.6)',
                  border: interactionType === tOpt.id ? '1px solid rgba(255,255,255,0.3)' : 'none',
                  fontWeight: 600
                }}>{t(`med_wiz_type_${tOpt.id}`) || (tOpt.id === 'v1' ? t("g_1deafd") || "\uBC14\uB514\uC2A4\uCE94" : tOpt.id === 'v2' ? t("g_555bfd") || "\uD638\uD761" : t("g_bb1252") || "\uC790\uC138")}</button>)}
                                </div>
                ...
                marginTop: '15px'
                  ...
                  fontSize: '0.75rem',
                  marginBottom: '8px'
                }}>🎵 {t('med_wiz_ambient_title_short') || t("g_e9eae1") || "\uBC30\uACBD\uC74C"}</div>
                                    <div style={{
                  display: 'flex',
                  gap: '6px',
                  flexWrap: 'wrap'
            ...
            marginTop: '30px'
              ...
              cursor: 'pointer',
              boxShadow: '0 10px 20px rgba(var(--primary-rgb), 0.3)'
            }}><Play size={24} weight="fill" /> {t('med_wiz_btn_start_now') || t("g_d31188") || "\uC2DC\uC791\uD558\uAE30"}</button>
              ...
              cursor: 'pointer',
              fontSize: '0.85rem'
            }}>{t('med_wiz_btn_reselect') || t("g_9a6547") || "\uB2E4\uC2DC \uC120\uD0DD (\uB300\uD654\uB85C \uB3CC\uC544\uAC00\uAE30)"}</button>
                        </div>
                    </div>
                </div>
            </div>;
