...
const ColorLegend = memo(({ branchId }) => {
...
const AdminScheduleManager = ({ branchId }) => {
    ...
    const {
        ...
        monthlyBookings, handlePrevMonth, handleNextMonth,
            ...
            <div style={{ overflowX: 'auto', paddingBottom: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', minWidth: '800px' }}>
                    ...
                    {dates.map((date, i) => {
                        ...
                        return (
                            <div key={dateStr} onClick={() => date && handleDateClick(dateStr)} style={{
                                minHeight: '100px',
                                ...
                                borderRadius: '8px', padding: '6px', cursor: date ? 'pointer' : 'default',
                                position: 'relative', zIndex: isToday ? 2 : 1
                            }}>
                                {date && (<>
                                    ...
                                    {holidayName && (
                                        <div style={{ fontSize: '0.65rem', color: '#ff4757', marginBottom: '4px', fontWeight: 'bold' }}>🎉 {t(holidayName)}</div>
                                    )}
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                        {classes.map((cls, idx) => {
                                            ...
                                            return (
                                                <div key={idx} style={{
                                                    fontSize: '0.8rem', padding: '4px 6px', borderRadius: '6px',
                                                    ...
                                                    <span style={{ fontWeight: 'bold' }}>{cls.time} {cls.title}</span>
                                                    ...
                                                    {allowBooking && (() => {
                                                        ...
                                                        return (
                                                            <span style={{ fontSize: '0.7em', fontWeight: 'bold', color: bookedCount >= capacity ? '#ff4757' : '#3B82F6', display: 'block' }}>
                                                                📋 {bookedCount}/{capacity}명
                                                            </span>
                                                        );
                                                    })()}
                                                </div>
    ...
    const renderUndefinedView = () => (
        <div style={{ padding: '0px' }}>
            <div style={{ marginBottom: '20px', padding: '24px', backgroundColor: 'rgba(59, 130, 246, 0.05)', borderRadius: '12px', border: '1px solid rgba(59, 130, 246, 0.2)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', textAlign: 'center' }}>
                <div>
                    <h3 style={{ fontSize: '1.2rem', color: '#60A5FA', margin: '0 0 8px 0' }}>{year}년 {month}월 일정이 비어 있습니다.</h3>
                    <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.95rem' }}>일일이 등록할 필요 없이, 지난 달 일정을 그대로 복사해서 편하게 시작하세요.</p>
                </div>
                <button onClick={handleCopyPrevMonth} style={{ padding: '14px 32px', fontSize: '1.05rem', backgroundColor: '#3B82F6', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(59, 130, 246, 0.3)', transition: 'all 0.2s' }}>
                    📥 지난달 스케줄 그대로 복사하기
                </button>
            </div>
            {renderCalendar()}
        ...
        <div style={{ padding: '20px', backgroundColor: 'var(--bg-surface)', borderRadius: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <button onClick={handlePrevMonth} className="nav-btn-circle" aria-label="이전 월"><CaretLeft /></button>
                    ...
                    <button onClick={handleNextMonth} className="nav-btn-circle" aria-label="다음 월"><CaretRight /></button>
                </div>
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    <button onClick={handleOpenRestore} className="action-btn-gold" style={{ backgroundColor: '#6366F1', opacity: 0.9 }}>
                        <ClockCounterClockwise size={18} /> 백업 복원
                    </button>
                    <button onClick={() => setShowSettings(true)} className="action-btn-gold" style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'var(--text-primary)', border: '1px solid var(--border-color)' }}>
                        <Gear size={18} /> 설정
                    </button>
                    {scheduleStatus === 'saved' && (
                        <button onClick={handleReset} className="action-btn-gold" style={{ backgroundColor: '#EF4444', opacity: 0.8 }}>
                            <Trash size={18} /> 초기화
                        </button>
                    )}
                </div>
            ...
            <div style={{ marginBottom: '15px' }}><ColorLegend branchId={branchId} /></div>
            ...
            {/* ── 수업 편집 모달 ── */}
            {showEditModal && (
                <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="수업 관리">
                    <div className="modal-content">
                        ...
                        <div style={{ maxHeight: '400px', overflowY: 'auto', paddingRight: '10px' }}>
                            ...
                            {dayClasses.map((cls, idx) => (
                                <ScheduleClassEditor key={idx} cls={cls} idx={idx} dayClasses={dayClasses} setDayClasses={setDayClasses} instructors={instructors} classTypes={classTypes} classLevels={classLevels} />
                            ))}
                            <button onClick={() => setDayClasses([...dayClasses, { time: '10:00', title: classTypes[0] || '수업', instructor: instructors[0] || '', status: 'normal', duration: 60, level: '' }])} className="action-btn-gold" style={{ width: '100%', marginTop: '8px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px dashed var(--border-color)', color: 'var(--text-secondary)' }}>
                                <Plus size={18} /> 수업 추가
                            </button>
                        </div>
                        <div style={{ display: 'flex', gap: '10px', marginTop: '24px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                            <button onClick={() => setShowEditModal(false)} className="action-btn-gold" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', border: '1px solid var(--border-color)' }}>취소</button>
                            <button onClick={() => handleSave(true)} className="action-btn-gold" style={{ backgroundColor: '#8B5CF6' }}>
                                📅 이 달의 모든 {new Date(selectedDate).toLocaleString('ko-KR', { weekday: 'short' })}요일 수정
                            </button>
                            <button onClick={() => handleSave(false)} className="action-btn-gold">저장</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* ── 초기화 확인 모달 ── */}
            {showResetConfirm && (
                <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="스케줄 초기화 확인">
                    <div className="modal-content" style={{ maxWidth: '400px', textAlign: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                            <button onClick={() => setShowResetConfirm(false)} className="action-btn-gold" style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', flex: 1 }}>취소</button>
                            <button onClick={confirmReset} className="action-btn-gold" style={{ backgroundColor: '#EF4444', flex: 1 }}>네, 초기화합니다</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* ── 백업 복원 모달 ── */}
            {showRestoreModal && (
                <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="백업 복원">
                    <div className="modal-content" style={{ maxWidth: '500px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '300px', overflowY: 'auto' }}>
                            {backupList.length === 0 ? (
                                <div style={{ textAlign: 'center', padding: '30px', backgroundColor: 'var(--bg-input)', borderRadius: '8px', color: 'var(--text-secondary)' }}>사용 가능한 백업이 없습니다.</div>
                            ) : (
                                backupList.map((backup, idx) => {
                                    ...
                                    return (
                                        <div key={backup.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', backgroundColor: 'var(--bg-input)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                            <div>
                                                <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>{date.toLocaleString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}에 백업됨</div>
                                            ...
                                            <button onClick={() => handleRestoreBackup(backup.id)} className="action-btn-gold" style={{ backgroundColor: '#6366F1', padding: '8px 16px', fontSize: '0.9rem' }}>이 지점으로 복원</button>
                                        </div>
                                    );
                                })
                            )}
