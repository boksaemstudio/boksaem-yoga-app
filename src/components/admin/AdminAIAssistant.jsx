...
const AdminAIAssistant = () => {
    ...
    useEffect(() => {
        if (historyLoaded && messages.length > 0) {
            try { sessionStorage.setItem('ai_assistant_messages', JSON.stringify(messages.slice(-30))); } catch { /* ignore */ }
        }
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const errMsg = error?.message || '오류가 발생했습니다.';
            const isQuotaError = errMsg.includes('한도') || errMsg.includes('resource-exhausted');
            setMessages(prev => [...prev, {
                role: 'assistant',
                content: isQuotaError
                    ? `⚠️ 오늘의 AI 비서 사용 한도(${quota.limit}회)를 모두 사용했습니다. 내일 다시 이용해주세요.`
    ...
    const getCategoryBadge = (category) => {
        const map = {
            member: { label: '회원', color: '#3B82F6' },
            attendance: { label: '출석', color: '#10B981' },
            revenue: { label: '매출', color: '#F59E0B' },
            schedule: { label: '스케줄', color: '#8B5CF6' },
            strategy: { label: '전략', color: '#EC4899' },
        ...
        return (
            <span style={{
                ...
                background: `${info.color}22`, color: info.color, fontWeight: 'bold',
                marginLeft: '6px'
            }}>{info.label}</span>
        );
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 200px)', maxHeight: '800px', minHeight: '400px' }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                        <div style={{ fontWeight: '800', fontSize: '1rem', color: 'var(--text-primary)' }}>AI 비서</div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    <button 
                        ...
                        style={{
                            ...
                            fontSize: '0.7rem', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px'
                        }}
                    >
                        <Clock size={14} /> 대화기록
                    </button>
            ...
            {showHistory && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                        <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--text-secondary)' }}>📋 과거 대화 기록</span>
                        <button onClick={() => setShowHistory(false)} style={{ 
                            background: 'none', border: 'none', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: '0.75rem'
                        }}>닫기 ✕</button>
                    </div>
                    {history.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-tertiary)', fontSize: '0.8rem', padding: '20px 0' }}>아직 대화 기록이 없습니다</div>
                    ) : (
                            ...
                            >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <span style={{ fontSize: '0.6rem', color: 'var(--text-tertiary)' }}>
                                        {item.timestamp ? new Date(item.timestamp).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                                    </span>
                                </div>
                    ...
                    <div style={{ 
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)', lineHeight: '1.6' }}>
                                이름·전화번호·종목으로 회원 검색, 출석·매출 분석까지<br/>
                                스튜디오의 <span style={{ color: 'var(--primary-gold)', fontWeight: 'bold' }}>모든 데이터</span>를 기반으로 답변합니다.<br/>
                                <span style={{ color: '#10B981', fontWeight: 'bold', display: 'inline-block', marginTop: '4px' }}>✨ 운영을 할수록 데이터가 쌓여 맞춤형 조언이 가능합니다.</span>
                            </div>
                        </div>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center', maxWidth: '420px' }}>
                            {[
                                ...
                                <button key={q} onClick={() => { setInput(q); setTimeout(() => { handleSend(); }, 100); }}
                                    style={{
                                        padding: '8px 14px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 'bold',
                                        background: 'rgba(var(--primary-rgb), 0.08)', color: 'var(--primary-gold)',
                                        border: '1px solid rgba(var(--primary-rgb), 0.2)', cursor: 'pointer',
                    ...
                    <div key={idx} style={{
                        ...
                        <div style={{
                            ...
                            background: msg.role === 'user'
                                ...
                                : 'linear-gradient(135deg, var(--primary-gold), #8B5CF6)',
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                        ...
                        <div style={{
                            ...
                            {msg.suggestions && msg.suggestions.length > 0 && (
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                                    {msg.suggestions.map((s, si) => (
                                        <button key={si} onClick={() => handleSuggestionClick(s)}
                                            style={{
                                                padding: '4px 10px', borderRadius: '12px', fontSize: '0.7rem',
                                                background: 'rgba(var(--primary-rgb), 0.08)', color: 'var(--primary-gold)',
                                                border: '1px solid rgba(var(--primary-rgb), 0.15)', cursor: 'pointer'
                                            }}
                                        >{s}</button>
                                    ))}
                                </div>
                            ...
                            <div style={{ display: 'flex', gap: '4px' }}>
                                {[0, 1, 2].map(i => (
                                    <div key={i} style={{
                                        ...
                                        background: 'var(--primary-gold)', opacity: 0.6,
                                        animation: `pulse 1.2s ease-in-out ${i * 0.2}s infinite`
                                    }} />
                                ))}
                            </div>
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>데이터 분석 중...</span>
                        </div>
                    </div>
                )}
            ...
            <div style={{
                ...
                {isListening && (
                    <div style={{
                        ...
                        <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#EF4444', animation: 'pulse 1s ease-in-out infinite' }} />
                        음성 입력 중... 말씀이 끝나면 자동으로 입력됩니다
                    </div>
                )}
            </div>
