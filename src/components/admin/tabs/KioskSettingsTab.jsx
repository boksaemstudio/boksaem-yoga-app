        ...
        <div className="dashboard-card" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ padding: '16px', background: 'rgba(var(--primary-rgb), 0.05)', borderRadius: '12px', border: '1px solid rgba(var(--primary-rgb), 0.2)', marginBottom: '24px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: '1.7' }}>
                    <p style={{ margin: '0 0 10px 0', color: 'var(--text-primary)', fontWeight: 'bold' }}>{t('키오스크 공지 기능 안내')}</p>
                    {t('출석체크 태블릿 화면에 이미지나 영상을 표시할 수 있습니다.')}
                    <br/><strong style={{ color: 'var(--primary-gold)' }}>{t('사용 방법:')}</strong>
                    ...
                    <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>{t('지원 파일: 이미지(최대 10MB) | 영상(최대 50MB, MP4/WebM/MOV)')}</span>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <div style={{
                        ...
                        {settings.imageUrl ? (
                            isVideoMedia(settings.imageUrl, settings.mediaType) ? (
                                ...
                                <img src={settings.imageUrl} alt={t('키오스크 공지')} style={{ width: '100%', height: 'auto', maxHeight: '400px', objectFit: 'contain' }} />
                            )
                        ) : (
                            <div style={{ color: 'var(--text-tertiary)', textAlign: 'center', padding: '40px' }}>
                                ...
                                <div style={{ fontSize: '0.8rem', marginTop: '4px', opacity: 0.6 }}>{t('위 버튼으로 새로 업로드하세요')}</div>
                            </div>
                        )}
                        {isUploading && (
                            <div style={{
                ...
                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '24px', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                    ...
                    {loadingGallery ? (
                        <div style={{ textAlign: 'center', padding: '30px', color: 'var(--text-tertiary)' }}>{t('불러오는 중...')}</div>
                    ) : gallery.length === 0 ? (
                ...
                <div style={{
                    ...
                    <h4 style={{ margin: 0, fontSize: '1rem', color: 'var(--text-primary)' }}>{t('⚙️ 공지 화면 옵션')}</h4>
                ...
                <div style={{
                    ...
                    <h4 style={{ margin: 0, fontSize: '1rem', color: 'var(--text-primary)' }}>{t('⏰ 키오스크 운영 시간 (자동 ON/OFF)')}</h4>
                    ...
                    {settings.autoOnOff && (
                        <div style={{ display: 'flex', gap: '16px', marginTop: '8px' }}>
                            <div style={{ flex: 1 }}>
                                <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'block', marginBottom: '8px' }}>{t('시작 시간 (켜짐)')}</label>
                                <input 
                                    type="time" 
                            ...
                            <div style={{ flex: 1 }}>
                                <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'block', marginBottom: '8px' }}>{t('종료 시간 (꺼짐)')}</label>
                                <input 
                                    type="time" 
