...
const KioskSettingsTab = () => {
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      try {
        ...
        const tenantList = await listAll(tenantFolder).catch(() => ({
          items: []
        }));
        const tenantItems = await parseItems(tenantList);
        const legacyList = await listAll(legacyFolder).catch(() => ({
          items: []
        }));
        const legacyItems = await parseItems(legacyList);
        const seenPaths = new Set(tenantItems.map(i => i.name));
        const mergedItems = [...tenantItems, ...legacyItems.filter(i => !seenPaths.has(i.name))];
  ...
  const handleImageUpload = e => {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      alert(`이미지 최대 ${MAX_IMAGE_SIZE / 1024 / 1024}MB`);
      return;
    }
  ...
  const handleVideoUpload = e => {
    ...
    if (file.size > MAX_VIDEO_SIZE) {
      alert(`영상 최대 ${MAX_VIDEO_SIZE / 1024 / 1024}MB`);
      return;
    }
  ...
  return <div className="dashboard-card" style={{
    ...
    margin: '0 auto'
              ...
              zIndex: 10
                ...
                color: 'var(--text-secondary)'
                  ...
                  cursor: 'pointer'
                                                    ...
                                                    <input type="radio" checked={currentBg === bgValue} onChange={async () => {
                    const newBgs = [...bgs];
                    ...
                    try {
                      ...
                      alert((t("g_f4eb20") || "\uBCC0\uACBD \uC2E4\uD328: ") + e.message);
                                        ...
                                        {/* 농도 조절 슬라이더 */}
                                        {currentBg !== 'transparent' && <div style={{
                padding: '0 16px',
                marginTop: '8px',
                width: '100%',
                boxSizing: 'border-box'
                  ...
                  ...prev,
                  [slotIdx]: parseFloat(e.target.value)
                }))} onMouseUp={async () => {
      ...
      alignItems: 'flex-start'
                ...
                <Info size={20} color="var(--primary-gold)" style={{
        flexShrink: 0,
        marginTop: '2px'
      }} />
                <div style={{
      ...
      gap: '24px'
        ...
        border: '1px solid var(--border-color)'
          ...
          marginBottom: '16px'
            ...
            gap: '8px'
              ...
              display: 'none'
            }} id="upload-kiosk-media" disabled={isUploading} />
          ...
          position: 'relative'
                        ...
                        {settings.imageUrl ? isVideoMedia(settings.imageUrl, settings.mediaType) ? <video src={settings.imageUrl} controls loop muted playsInline style={{
            width: '100%',
            height: 'auto',
            maxHeight: '400px',
            objectFit: 'contain'
            ...
            padding: '40px'
                                ...
                                <FilmSlate size={48} weight="light" style={{
              marginBottom: '8px',
              opacity: 0.5
            }} />
        ...
        border: '1px solid var(--border-color)'
          ...
          gap: '12px'
                            ...
                            {gallery.map(item => <div key={item.url} style={{
            borderRadius: '12px',
            overflow: 'hidden',
            position: 'relative',
            ...
            boxShadow: item.isActive ? '0 0 12px rgba(var(--primary-rgb), 0.3)' : 'none'
              ...
              position: 'relative'
                                        ...
                                        {item.type === 'video' ? <video src={item.url} muted preload="metadata" style={{
                width: '100%',
                height: '100%',
                objectFit: 'cover'
              }} onLoadedData={e => {
        ...
        justifyContent: 'space-between',
        alignItems: 'center',
        opacity: settings.imageUrl ? 1 : 0.5,
        pointerEvents: settings.imageUrl ? 'auto' : 'none'
      }}>
                    <div>
                        <h4 style={{
            margin: '0 0 4px 0',
        ...
        flexDirection: 'column',
        gap: '16px',
        opacity: settings.active ? 1 : 0.5,
        pointerEvents: settings.active ? 'auto' : 'none'
      }}>
                    <h4 style={{
          margin: 0,
          fontSize: '1rem',
