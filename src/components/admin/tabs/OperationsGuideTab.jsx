    ...
    return (
        <div style={{ maxWidth: '700px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '20px 0 10px' }}>
                <h2 style={{ color: 'white', fontSize: '1.3rem', fontWeight: '800', margin: '0 0 6px' }}>{t('📖 운영 가이드')}</h2>
                <p style={{ color: '#71717a', fontSize: '0.85rem', margin: 0 }}>{t('이 앱의 모든 자동화 규칙과 정책을 한 눈에 확인하세요')}</p>
            </div>
            ...
            {openSections.members && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '4px' }}>
                    <RuleCard
                        ...
                        title={t('출석 체크 & 횟수 차감')}
                        description={t('키오스크에서 회원이 출석하면 잔여 횟수가 자동으로 1회 차감됩니다. 관리자가 수동 출석 처리 시에는 차감 여부를 선택할 수 있습니다.')}
                        tip={t('같은 날 여러 수업 출석도 가능합니다(다중 출석). 각 출석마다 1회씩 차감됩니다.')}
                    />
                    <RuleCard
                        icon={<Warning size={20} weight="fill" color="#ef4444" />}
                        title={t('출석 거부 (자동 차단)')}
                        description={t('기간이 만료(종료일 초과)되었거나 잔여 횟수가 0인 회원은 키오스크에서 자동으로 출석이 거부됩니다. 화면에 \'기간 만료\' 또는 \'횟수 부족\' 사유가 표시됩니다.')}
                        tip={t('관리자 대시보드에서는 거부된 출석 시도도 기록으로 남아 확인할 수 있습니다.')}
                    />
                    <RuleCard
                        icon={<Clock size={20} weight="fill" color="#3b82f6" />}
                        title={t('TBD: 시작일 미확정 등록')}
                        description='회원 등록 시 시작일을 "TBD(미정)"로 설정할 수 있습니다. 이 회원이 처음 출석하는 날이 자동으로 시작일이 되고, 수강 기간에 따라 종료일이 자동 계산됩니다.'
                        badge={t('첫 출석 시 자동 확정')}
                        tip={t('예: 3개월 수강권을 TBD로 등록 → 4월 1일 첫 출석 → 시작일 4/1, 종료일 6/30 자동 설정')}
                    />
                    <RuleCard
                        ...
                        tip={t('예: 7일 홀딩 후 출석 → 종료일이 7일 뒤로 자동 연장')}
                    />
                    <RuleCard
                        icon={<CalendarCheck size={20} weight="fill" color="#8b5cf6" />}
                        title={t('선등록 (미래 수강권 예약)')}
                        description={t('현재 수강권이 진행 중인 회원에게 다음 수강권을 미리 등록할 수 있습니다. 기존 횟수가 소진되거나 기간이 만료되면 첫 출석 시 선등록 수강권이 자동 활성화됩니다. 활성화 시 기존 잔여 횟수는 무조건 0이 되고, 새 수강권의 횟수와 기간으로 교체됩니다.')}
                        badge={t('소진/만료 시 자동 전환')}
                        tip={t('선등록 수강권은 시작일 옵션(TBD/직접지정/즉시출석)에 따라 종료일이 산정됩니다.')}
                    />
                    <RuleCard
            ...
            {openSections.membership && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '4px' }}>
                    <RuleCard
                        ...
                        title={t('종료일 산정 기준')}
                        description={t('종료일은 항상 \'시작일 + 수강권 기간(개월) - 1일\'로 자동 계산됩니다. 예를 들어 시작일이 3월 25일이고 3개월 수강권이면 종료일은 6월 24일입니다.')}
                        badge={t('시작일 기준 자동 계산')}
                        tip={t('예: 3/25 + 1개월 → 4/24 | 3/25 + 3개월 → 6/24 | 4/1 + 3개월 → 6/30')}
                    />
                    <RuleCard
                        icon={<ArrowsClockwise size={20} weight="fill" color="#3b82f6" />}
                        title={t('횟수 0 + 기간 남음 → 재등록')}
                        description={t('잔여 횟수가 0이지만 종료일이 아직 남아있는 경우 재등록하면, 선택한 시작일부터 새 수강권 기간만큼 종료일이 산정됩니다. 기존의 남은 기간은 무시됩니다.')}
                        badge={t('시작일부터 새로 계산')}
                    />
                    <RuleCard
                        icon={<ArrowsClockwise size={20} weight="fill" color="#f59e0b" />}
                        title={t('횟수 남음 + 기간 종료 → 재등록')}
                        description={t('기간이 만료되었지만 잔여 횟수가 남아있는 경우 재등록하면, 기존 잔여 횟수는 삭제(0)되고 새 수강권의 횟수가 부여됩니다. 종료일도 새 수강권 기간에 따라 새로 산정됩니다.')}
                        badge={t('기존 횟수 삭제 → 새로 부여')}
                        tip={t('예: 기존 5회 남았지만 기간 만료 → 재등록 시 새 수강권 35회로 교체')}
                    />
                    <RuleCard
                        icon={<ArrowsClockwise size={20} weight="fill" color="#ef4444" />}
                        title={t('횟수 0 + 기간 종료 → 재등록 (신규와 동일)')}
                        description={t('횟수도 0이고 기간도 지난 경우 재등록하면, 신규 등록과 완전히 동일하게 처리됩니다. 선택한 시작일부터 수강권 기간에 따라 종료일이 산정되고 새 횟수가 부여됩니다.')}
                        badge={t('신규 등록과 동일')}
                    />
                    <RuleCard
                        icon={<CalendarCheck size={20} weight="fill" color="#8b5cf6" />}
                        title={t('선등록 시 회원권 전환')}
                        description={t('현재 수강권이 활성 상태인 회원에게 선등록하면, 기존 횟수가 소진되거나 기간이 만료되었을 때 첫 출석 시 자동으로 전환됩니다. 전환 시 기존 잔여 횟수는 0이 되고, 새 수강권의 횟수와 기간으로 완전히 교체됩니다.')}
                        badge={t('기존 횟수 사라짐')}
                        tip={t('시작일 옵션(TBD/직접지정/즉시출석)에 따라 종료일이 산정됩니다. 어떤 경우든 기존 횟수는 보존되지 않습니다.')}
                    />
                </div>
            )}
            ...
            {openSections.booking && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '4px' }}>
                    ...
                    <RuleCard
                        ...
                        tip={t('매시간 자동으로 체크합니다. 수업 시간이 지나야 노쇼로 처리됩니다.')}
                    />
                    <RuleCard
                        icon={<Clock size={20} weight="fill" color="#f59e0b" />}
                        title={t('취소 마감 시간')}
            ...
            {openSections.alerts && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '4px' }}>
                    <RuleCard
                        icon={<Bell size={20} weight="fill" color="#f43f5e" />}
                        title={t('크레딧 소진 알림')}
                        description={t('회원의 잔여 횟수가 0이 되면 AI가 해당 회원의 수강 패턴을 분석하여 맞춤형 알림 메시지를 자동 생성합니다. 관리자가 승인하면 회원에게 푸시 알림으로 발송됩니다.')}
                        tip={t('자동 발송이 아닌 \'관리자 승인 후 발송\' 방식이므로 안심하셔도 됩니다.')}
                    />
                    <RuleCard
                        icon={<ShieldCheck size={20} weight="fill" color="#10b981" />}
                        title={t('일일 리포트 (매일 23시)')}
                        description={t('매일 밤 11시에 오늘의 출석 수, 신규 가입 수, 보안 이상 여부를 정리한 요약 보고서가 관리자에게 푸시 알림으로 발송됩니다.')}
                        badge={t('매일 23:00')}
                    />
                    <RuleCard
                        icon={<Clock size={20} weight="fill" color="#6366f1" />}
                        title={t('예약 메시지 자동 발송')}
                        description={t('관리자가 예약한 알림 메시지(수업 리마인더, 이벤트 안내 등)는 10분 간격으로 자동 발송됩니다. 푸시 알림 우선이며, 푸시 실패 시 SMS로 대체 발송합니다.')}
                        badge={t('10분 주기')}
                        tip={t('푸시 → SMS 순서로 발송합니다. 앱 미설치 회원에게도 SMS로 전달됩니다.')}
                    />
                </div>
            )}
            ...
            {openSections.automation && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '4px' }}>
                    <RuleCard
                        ...
                        title={t('AI 대시보드 브리핑')}
                        description={t('관리자 대시보드에 접속하면 AI가 현재 시간대에 맞는 전략적 분석을 제공합니다. 활성 회원 수, 매출, 출석률 등 실시간 데이터를 기반으로 실행 가능한 인사이트를 생성합니다.')}
                        tip={t('AI는 제공된 데이터의 정확한 수치만 사용하며, 임의의 숫자를 생성하지 않습니다.')}
                    />
                    <RuleCard
                        icon={<ShieldCheck size={20} weight="fill" color="#10b981" />}
                        title={t('데이터 자동 백업')}
                        description={t('매일 오전과 오후, 하루 2회 Firestore 데이터가 자동으로 백업됩니다. 데이터 손실 시 백업 시점으로 복원할 수 있습니다.')}
                        badge={t('1일 2회')}
                    />
                    <RuleCard
                        icon={<Warning size={20} weight="fill" color="#f59e0b" />}
                        title={t('유령 토큰 자동 정리')}
                        description={t('60일 이상 업데이트되지 않은 만료된 푸시 알림 토큰을 자동으로 정리합니다. 이를 통해 불필요한 알림 발송 시도를 줄이고 시스템 효율을 유지합니다.')}
                        badge={t('60일 초과 시 삭제')}
                    />
                </div>
            )}
            ...
            <div style={{ textAlign: 'center', padding: '20px 0', color: '#52525b', fontSize: '0.78rem' }}>
                ...
                <p>{t('설정 변경은')} <strong style={{ color: '#a1a1aa' }}>{t('⚙️ 설정')}</strong> {t('탭에서 할 수 있습니다.')}</p>
            </div>
        </div>
    );
};
