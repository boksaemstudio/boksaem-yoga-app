...
const CustomDatePicker = ({
      ...
      padding: '10px'
        ...
        marginBottom: '10px'
          ...
          gap: '5px'
                        ...
                        <select value={year} onChange={handleYearChange} style={headerSelectStyle} onClick={e => e.stopPropagation()}>
                            {Array.from({
              length: 21
            }, (_, i) => year - 10 + i).map(y => <option key={y} value={y}>{y}{t("g_e29d2c") || "\uB144"}</option>)}
                        </select>
                        <select value={month} onChange={handleMonthChange} style={headerSelectStyle} onClick={e => e.stopPropagation()}>
                            {Array.from({
              length: 12
            }, (_, i) => i).map(m => <option key={m} value={m}>{m + 1}{t("g_5b51dd") || "\uC6D4"}</option>)}
                        </select>
                    </div>
        ...
        textAlign: 'center'
                    ...
                    {[t("g_95e431") || "\uC77C", t("g_5b51dd") || "\uC6D4", t("g_74d3f7") || "\uD654", t("g_cae82d") || "\uC218", t("g_d5f699") || "\uBAA9", t("g_cef92d") || "\uAE08", t("g_ccc0dc") || "\uD1A0"].map((d, i) => <div key={d} style={{
          fontSize: '0.8rem',
          fontWeight: 'bold',
          color: i === 0 ? '#ef4444' : '#a1a1aa',
          padding: '5px 0'
        }}>{d}</div>)}
                    {weeks.map((week, wIdx) => week.map((date, dIdx) => {
          ...
          return <div key={dateStr} onClick={() => handleDateClick(date)} style={{
            padding: '8px 0',
            fontSize: '0.9rem',
            cursor: 'pointer',
            borderRadius: '6px',
  ...
  return <div ref={containerRef} style={{
    ...
    width: '100%'
      ...
      fontSize: '1rem',
      cursor: 'pointer',
      fontFamily: 'Pretendard, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", sans-serif'
    }}>
                <span>{value === 'TBD' ? t("g_b8e060") || "\uCCAB \uCD9C\uC11D \uC2DC \uD655\uC815" : value || placeholder}</span>
