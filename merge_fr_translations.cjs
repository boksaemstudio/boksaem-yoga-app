...
const incomingPath = path.join(__dirname, 'incoming_fr_chunk.json');
const incomingJson = JSON.parse(fs.readFileSync(incomingPath, 'utf8'));

// Provide fallback keys just in case 
...
// safely insert right after "fr: {"
tsContent = tsContent.replace(/fr\s*:\s*\{/, `fr: {\n${insertStr}`);

fs.writeFileSync(tsPath, tsContent, 'utf8');
console.log(`Safely merged ${Object.keys(incomingJson).length} keys into FR dictionary!`);
